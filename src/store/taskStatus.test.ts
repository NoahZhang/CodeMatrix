...
// Mock the Zustand store before importing the module under test.
let mockAutoTrustFolders = false;
let mockActiveTaskId: string | null = null;
vi.mock('./store', () => ({
  getStore: () => ({
    autoTrustFolders: mockAutoTrustFolders,
...
// Mock IPC so tryAutoTrust's invoke call doesn't hit Tauri.
vi.mock('../lib/ipc', () => ({
